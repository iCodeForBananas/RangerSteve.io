...
export function init(primusInstance) {
  io = primusInstance;
  Server.init(io);
...
roomUpdateLoop.on("update", function () {
  Object.keys(rooms).forEach(roomId => {
    ...
    if (rooms[roomId].roundStartTime <= now && rooms[roomId].state === "ended") {
      ...
      console.log(
        `${rooms[roomId].map} has been selected to play ${rooms[roomId].gamemode} with ${GameConsts.MODES[rooms[roomId].mode]
        } for room ${roomId}`
      );
...
export function getHighscorePlayer(players) {
...
export function removePlayer(id, roomId) {
...
export function onPlayerScores() {
  const roomId = getRoomIdByPlayerId(this.id, rooms);
  if (!rooms[roomId]) return;
...
export function onMessageSend(data) {
  const roomId = getRoomIdByPlayerId(this.id, rooms);
  if (!rooms[roomId]) return;
...
export function onNewPlayer(data) {
...
export function onPlayerFullHealth() {
  const roomId = getRoomIdByPlayerId(this.id, rooms);
  if (!rooms[roomId]) return;
...
export function onPlayerHealing() {
  const roomId = getRoomIdByPlayerId(this.id, rooms);
  if (!rooms[roomId]) return;
...
export function onPlayerDamaged(data) {
  const roomId = getRoomIdByPlayerId(this.id, rooms);
  if (!rooms[roomId]) return;
...
export const rangeOfVariance = _.range(-7, 7, 1);

export function onShooting(player, roomId) {
  const data = {
    bullets: [],
    playerId: this.id,
